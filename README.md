...
- **`sharedLogic`** — the Kotlin Multiplatform module (`commonMain` + `androidMain` + `iosMain`), with no UI dependencies at all. It owns:
  ...
  - Persistence: SQLDelight, with hand-rolled migration files (`2.sqm`, `3.sqm`) tracked alongside the `.sq` schema
...
Favorites live in a separate SQLDelight table, observed reactively (`observeFavorites()`, `observeIsFavorite(id)`). Android's ViewModels subscribe to that `Flow` directly, so a toggle shows up on every screen with no manual refresh wiring. On iOS, `FavoritesClient` only exposes one-shot snapshot calls (`getFavorites()`, `isFavorite()`) rather than a stream, so I have `AppFeature` explicitly resync the other tab (re-dispatching `.favorites(.task)` / `.popularMovies(.refreshFavoriteIds)`) whenever a favorite toggle succeeds from the other tab's own screen or navigation stack — otherwise a change made in one tab wouldn't show up in the other until it happened to reload on its own.
...
- **KMP-NativeCoroutines for the Flow → AsyncThrowingStream bridge**: `sharedLogic` exposes idiomatic Kotlin `Flow<Result<T>>`. Rather than inventing my own callback- or completion-handler-based Swift API, I used KMP-NativeCoroutines to generate a native `async`-compatible wrapper, so iOS effects can `for try await` over the exact same stream Android collects.
...
- **SQLDelight over Room**: Room isn't multiplatform. SQLDelight generates typed Kotlin from `.sq` files for both the Android (via `AndroidSqliteDriver`) and iOS (via `NativeSqliteDriver`) targets from one schema, and I version plain `.sqm` migration files alongside it.
